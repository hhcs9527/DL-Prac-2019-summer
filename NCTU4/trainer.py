...
class Trainer:
  ...
  def train(self):
    ...
    optimD = optim.Adam([{'params':self.FE.parameters()}, {'params':self.D.parameters()}], lr=0.0002, betas=(0.5, 0.9))
    optimG = optim.Adam([{'params':self.G.parameters()}, {'params':self.Q.parameters()}], lr=0.001, betas=(0.5, 0.9))
    ...
    for epoch in range(self.Ep):
      ...
      z = f.fix_noise_cat(self.batch_size, 'training', epoch)
      x_save = self.G(z)
      name = './try_ans/'+ '#epoch' + str(epoch+1)  +'.png'
      save_image(x_save.data, name, nrow=10)


      torch.save(self.G.state_dict(), self.get_path('G1'))

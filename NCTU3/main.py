...
if __name__ == '__main__':
    ...
    if (int(input()) == 1):
        T.trainIters(encoder1, decoder1, n_iters = 10, epoch = 3)
    else:
        print('Test result of tense transform : ')
        test.go_test(encoder1, decoder1)
        print('Testing is done !')
        #print('Testing result of the guassin noise generation : ')
    ## First we need to get the mu/logvar to build the noise
        #encoder_output, encoder_hidden, mu, logvar = encoder1(input_tensor[i], encoder_hidden, cond)
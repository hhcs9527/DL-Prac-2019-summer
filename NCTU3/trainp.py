...
def train(input_tensor, target_tensor,encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, cond ,input_lang, max_length=MAX_LENGTH):
    ...
    if use_teacher_forcing:
        ...
        for di in range(target_length):
            embed_decoder_input = embed(target_tensor[di], vocab_size, hidden_size + 10)
            decoder_output, decoder_hidden, decoder_predict = decoder(embed_decoder_input, decoder_hidden)
            decoder_input = target_tensor[di]  # Teacher forcing
            loss += criterion(decoder_predict, target_tensor[di])#torch.tensor([input_tensor[di]], dtype = torch.float32))
            #print('add loss ', criterion(decoder_predict, target_tensor[di]))
        ...
        for di in range(target_length):
            embed_decoder_input = embed(target_tensor[di], vocab_size, hidden_size + 10 )
            decoder_output, decoder_hidden, decoder_predict = decoder(embed_decoder_input, decoder_hidden)
            decoder_input = decoder_output  # Teacher forcing
            loss += criterion(decoder_predict, target_tensor[di])#torch.tensor([input_tensor[di]], dtype = torch.float32))
            #print('add loss ',criterion(decoder_predict, target_tensor[di])
...
def trainIters(encoder, decoder, n_iters, epoch, print_every=10, plot_every=10, learning_rate=0.001):
    start = time.time()
    plot_losses = []
...
## Training
    for i in range(epoch):
            ...
            for j in range(int(len(data)/n_iters)):
                ...
                for iter in range(1, n_iters + 1):
                    ...
                    if iter % print_every == 0:
                        ...
                        msg = '# epoch : {} # Tense change into : {} # Batch :{} iter : {:.2f}%  Loss_avg : {:.5f}'.format(i,Condition[c] ,j, iter / n_iters * 100, print_loss_total/count)
                        #print_loss_total = 0
                        #plot_loss_total = 0
                        print(msg)
        ...
        print('# epoch : {} Loss_avg = {} '. format(i, print_loss_total/len(data)/4))
